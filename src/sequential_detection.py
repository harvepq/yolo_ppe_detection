...
class PPEMonitoring:
  ...
  def predict(self, frame):
    """"Run detection using a YOLOv8 model for the input image 'frame'"""
  ...
  def __call__(self):
    ...
    assert cap.isOpened()
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, self.config["imgsz"][0])
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, self.config["imgsz"][1])
    while True:
      self.start_time = time.time()
      ret, frame = cap.read()
      if not ret:
        print("Frame was not capture.")
        continue
      ...
      cv2.imshow("PPE Monitoring - Sequential", frame)
...
if __name__ == "__main__":
  try:
    ...
    camera = 0
    detector = PPEMonitoring(camera, "models/trained/best.pt", [1,2], device=0, imgsz=(1536, 864), conf=0.8, iou=0.7, verbose=False)
    detector()
  except:
